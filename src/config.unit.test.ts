...
describe("resolveComponents", () => {
  it("returns the 5 components in fixed order with defaults", () => {
    const comps = resolveComponents(baseEnv);
    expect(comps.map((c) => c.key)).toEqual(["api", "rest", "webdav", "s3", "sftp"]);
    expect(COMPONENT_ORDER).toEqual(["api", "rest", "webdav", "s3", "sftp"]);
  ...
  it("honors env overrides and strips trailing slash on API_BASE", () => {
    const env = { API_BASE: "https://api.example.com/", SFTP_PROBE_HOST: "sftp.example.com", SFTP_PROBE_PORT: "2222" } as unknown as Env;
